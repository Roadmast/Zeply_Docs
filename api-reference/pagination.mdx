...
API_KEY = os.getenv("PINGR_API_KEY")
BASE_URL = "https://api.pingr.ai/api/v1/calls"
...
async function fetchAllAgents() {
  ...
  while (hasMore) {
    const url = new URL('https://api.pingr.ai/api/v1/agents');
    url.searchParams.set('limit', '50');
    if (cursor) url.searchParams.set('starting_after', cursor);

    const res = await fetch(url.toString(), {
      headers: { 'Authorization': `Bearer ${process.env.PINGR_API_KEY}` }
    });
